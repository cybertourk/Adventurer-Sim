...
import { Clock, HelpCircle, Minus, Plus, Sun, X, Shield, Hammer, Scroll, Zap, Heart, User, Coins, DollarSign, Activity, Tent, Droplets, Beer, Skull, Utensils } from 'lucide-react';
...
const StatBlock = ({ label, value, max, alert, inverted, onClick, subValue }) => (
    <button onClick={onClick} className={`flex flex-col items-center justify-center w-10 h-10 md:w-14 md:h-14 bg-slate-800/60 rounded-lg border backdrop-blur-sm shadow-sm transition-all hover:scale-105 active:scale-95 ${alert ? 'border-red-500/50 bg-red-900/10' : inverted ? 'border-amber-500/50 bg-amber-900/10' : 'border-slate-700/50 hover:border-slate-500'}`}>
...
const InnRoomBackground = () => (
  <div className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none bg-[#1a120b]">
    ...
    <div className="absolute top-10 left-10 w-24 h-32 bg-[#0f172a] border-4 border-[#3f2e18] rounded-t-full overflow-hidden">
       ...
       <div className="absolute bottom-0 w-full h-1 bg-black/50" /><div className="absolute w-1 h-full left-1/2 bg-[#3f2e18]" />
...
const App = () => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden text-slate-200 font-sans selection:bg-indigo-500/30">
      ...
      <div className="absolute inset-0 flex flex-col pointer-events-none p-4 pb-0 md:p-8">
        
        {/* Messages Toast */}
        <div className="absolute top-4 left-1/2 -translate-x-1/2 flex flex-col gap-2 z-50 pointer-events-auto w-full max-w-sm px-4">
          {messages.map(m => (
        ...
        {/* Top Header */}
        <header className="pointer-events-auto bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4 shadow-xl shadow-black/20">
            <div className="flex items-center gap-4">
                <div className="flex flex-col">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Time</span>
                    <span className="text-xl font-bold text-indigo-400">Day {days}</span>
                </div>
                ...
                <div className="flex flex-col">
                    ...
                    <span className="text-sm font-bold text-slate-200">{LOCATIONS[location]?.name}</span>
                </div>
                <div className="w-px h-8 bg-slate-700/50 hidden md:block"></div>
                <div className="flex flex-col">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Housing</span>
                    <span className={`text-sm font-bold ${housing === 'homeless' ? 'text-amber-500' : 'text-emerald-400'}`}>
                        {housing === 'inn' ? 'Inn Room' : housing === 'estate' ? 'Estate' : 'Homeless'}
                    </span>
                </div>
            </div>
            
            <div className="flex items-center gap-6 bg-slate-950/50 px-4 py-2 rounded-xl border border-slate-800/50">
                <div className="flex items-center gap-2">
                    <div className="p-1.5 bg-amber-500/20 rounded-md text-amber-400"><Coins size={16} /></div>
                    <span className="text-lg font-mono font-bold text-amber-400">{resources.gold}g</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className="p-1.5 bg-cyan-500/20 rounded-md text-cyan-400"><Activity size={16} /></div>
                    <div className="flex flex-col">
                        <span className="text-xs font-bold text-cyan-400">Lv {resources.level}</span>
                        <span className="text-[9px] text-cyan-200/50 font-mono">{resources.xp}/{resources.level*100}</span>
                    </div>
                </div>
            </div>
        </header>
        ...
        {/* Main Content Area */}
        <div className="flex-1 mt-4 flex flex-col lg:flex-row gap-4 min-h-0 relative">
          
          {/* Left Column: Character & Stats */}
          <div className="pointer-events-auto flex flex-col gap-4 w-full lg:w-1/3 max-w-sm mx-auto lg:mx-0">
             <div className="relative bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-2xl flex-1 flex flex-col items-center justify-center p-6 shadow-xl shadow-black/20 overflow-hidden min-h-[300px]">
                {quirk && (
                   <div className="absolute top-4 left-4 bg-indigo-900/50 border border-indigo-500/30 text-indigo-300 text-[10px] px-2 py-1 rounded-md font-bold max-w-[50%] z-10 truncate">
                      Trait: {quirk.name}
                   </div>
                )}
                {isDead && (
                   <div className="absolute inset-0 bg-red-950/80 z-20 flex flex-col items-center justify-center backdrop-blur-sm">
                      ...
                      <button onClick={revive} className="px-6 py-2 bg-red-600 hover:bg-red-500 text-white font-bold rounded-lg shadow-lg">Revive (Cost: 50 XP)</button>
                   </div>
                )}
             ...
             <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-4 shadow-xl shadow-black/20 flex flex-wrap justify-center gap-2 md:gap-3">
          ...
          <div className="pointer-events-auto flex-1 bg-slate-900/80 backdrop-blur-xl border border-slate-700/50 rounded-t-2xl lg:rounded-2xl shadow-xl shadow-black/20 flex flex-col overflow-hidden min-h-[400px]">
             
             {/* Tabs */}
             <div className="flex border-b border-slate-700/50 bg-slate-950/30 overflow-x-auto no-scrollbar">
                {['actions', 'inventory', 'shop', 'log'].map(tab => (
                    <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-4 px-6 text-xs font-bold uppercase tracking-widest whitespace-nowrap transition-colors ${activeTab === tab ? 'bg-indigo-600/20 text-indigo-400 border-b-2 border-indigo-500' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/30'}`}>
                        {tab}
                    </button>
                ))}
             </div>
             ...
             <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
                 ...
                 {activeTab === 'actions' && (
                    <>
                       <div>
                          <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2"><Tent size={14}/> Maintenance</h3>
                 ...
                 {activeTab === 'inventory' && (
                    <div className="space-y-6">
                        <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {Object.entries(equipped).map(([slot, itemId]) => {
                                    const item = ITEM_DB[slot].find(i => i.id === itemId);
                                    return (
                                        <div key={slot} className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-3 flex flex-col items-center justify-center text-center">
                                            <span className="text-[10px] text-slate-500 uppercase font-bold mb-1">{slot}</span>
                                            <span className="text-sm font-bold text-slate-300">{item ? item.name : 'None'}</span>
                        ...
                        <div>
                            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Bag</h3>
                            {inventory.filter(id => id !== 'none' && id !== 'fist').length === 0 ? (
                                <p className="text-sm text-slate-500 italic">Inventory is empty.</p>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    {inventory.filter(id => id !== 'none' && id !== 'fist').map((itemId, idx) => {
                                        ...
                                        return (
                                            <div key={idx} className="bg-slate-800 border border-slate-700 rounded-lg p-3 flex flex-col">
                                                <div className="flex justify-between items-start mb-2">
                                                    <div>
                                                        <span className="font-bold text-sm text-slate-200 block">{item.name}</span>
                                                        <span className="text-[10px] text-slate-500">{item.category}</span>
                                                    </div>
                                                    <span className="text-[10px] font-mono text-amber-400 bg-amber-900/20 px-2 py-0.5 rounded border border-amber-500/20">{Math.floor(item.cost/2)}g Value</span>
                                                </div>
                                                {renderItemStats(item)}
                                                <div className="mt-3 flex gap-2">
                                                    {['head', 'body', 'mainHand', 'offHand'].includes(item.type) && (
                                                        <button onClick={() => equipItem(item)} disabled={isEquipped} className={`flex-1 py-1.5 rounded text-xs font-bold transition-colors ${isEquipped ? 'bg-indigo-900/50 text-indigo-300 border border-indigo-500/30' : 'bg-slate-700 text-slate-300 hover:bg-indigo-600 hover:text-white'}`}>
                                                            {isEquipped ? 'Equipped' : 'Equip'}
                                                        </button>
                                                    )}
                                                    {(item.type === 'food' || item.type === 'drink' || item.type === 'potion') && (
                                                        <button onClick={() => consumeItem(item)} className="flex-1 py-1.5 bg-emerald-900/30 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-600 hover:text-white rounded text-xs font-bold transition-colors">
                                                            Consume
                                                        </button>
                                                    )}
                                                    {!isEquipped && (
                                                        <button onClick={() => sellItem(item)} className="px-3 py-1.5 bg-slate-800 border border-slate-600 text-slate-400 hover:bg-amber-600 hover:text-white hover:border-amber-500 rounded text-xs font-bold transition-colors">
                                                            Sell
                                                        </button>
                                                    )}
                                                </div>
                 ...
                 {activeTab === 'shop' && (
                    <div>
                        <div className="flex justify-between items-end mb-4">
                            ...
                            <button onClick={() => passTime(1)} className="text-[10px] bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700 hover:bg-slate-700 text-slate-300 font-bold transition-colors">
                                Skip Day & Refresh (1d)
                            </button>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {shopStock.map(itemId => {
                                ...
                                return (
                                    <div key={itemId} className={`bg-slate-800 border rounded-lg p-3 flex flex-col justify-between ${canAfford ? 'border-slate-600 hover:border-indigo-500' : 'border-slate-700 opacity-60'}`}>
                                        <div>
                                            <div className="flex justify-between items-start mb-2">
                                                <div>
                                                    <span className="font-bold text-sm text-slate-200 block">{item.name}</span>
                                                ...
                                                <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${canAfford ? 'text-amber-400 bg-amber-900/20 border-amber-500/20' : 'text-red-400 bg-red-900/20 border-red-500/20'}`}>
                                                    {cost}g
                                                </span>
                                            </div>
                                        ...
                                        <button onClick={() => buyItem(item)} disabled={!canAfford || isDead} className={`mt-3 w-full py-2 rounded text-xs font-bold transition-colors ${canAfford && !isDead ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 hover:bg-indigo-600 hover:text-white' : 'bg-slate-900 text-slate-600 cursor-not-allowed'}`}>
                                            Purchase
                                        </button>
                                    </div>
                                );
                 ...
                 {activeTab === 'log' && (
                    <div className="space-y-3 pb-8">
                        {dailyLogs.length === 0 ? (
                            ...
                            dailyLogs.map(log => (
                                <div key={log.id} className="bg-slate-800/40 border border-slate-700/50 rounded-lg p-3">
                                    <div className="flex justify-between items-center mb-2">
                                        <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded ${log.type === 'morning' ? 'bg-amber-900/30 text-amber-400' : 'bg-slate-900 text-slate-400'}`}>
                                            {log.type === 'morning' ? 'Morning Report' : 'Action'}
                                        </span>
                                        <span className="text-[10px] font-mono text-slate-500">Day {log.day}</span>
                                    </div>
                                    <p className="text-sm font-bold text-slate-300">{log.title || log.incidentTitle}</p>
                                    <p className="text-xs text-slate-400 mt-1">{log.text || log.incidentText}</p>
                                    {(log.changes || log.status !== 'Success') && (
                                        <div className={`mt-2 text-[10px] font-bold ${log.status === 'Failed' ? 'text-red-400' : 'text-indigo-400'}`}>
                        ...
                        <div className="flex justify-center mt-6">
                            <button onClick={resetGame} className="text-[10px] text-red-500/50 hover:text-red-400 font-bold uppercase tracking-widest underline decoration-red-500/30 underline-offset-4">Hard Reset Game</button>
                        </div>
                    </div>
                 )}
             </div>
