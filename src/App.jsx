...
import { 
  ...
  ClipboardList, Clock, Droplets, Tent, Hammer, Zap, Utensils, Plus, Minus 
} from 'lucide-react';
...
export const LOCATIONS = {
  village_road: {
    ...
    tips: [ { label: "Health", text: "Recovers only 5 (instead of 15).", type: "bad" }, { label: "Stress", text: "Reduces by only 5 (instead of 20).", type: "bad" }, { label: "Mood", text: "Decreases by 10.", type: "bad" } ],
  ...
  inn_room: {
    ...
    tips: [ { label: "Rest", text: "Full recovery benefits.", type: "good" }, { label: "Cost", text: "5 Gold deducted daily automatically.", type: "bad" } ],
  ...
  estate: {
    ...
    tips: [ { label: "Rest", text: "Maximum recovery benefits.", type: "good" }, { label: "Cost", text: "50 Gold deducted daily.", type: "bad" } ],
...
const ICONS = {
  Shield, Sword, Scroll, Activity, Apple, Beer, User, Coins, Heart, DollarSign, Droplets, Tent, Hammer, Zap, Utensils
};
...
const getBackground = (locationId) => {
  return BACKGROUND_MAP[locationId] || (() => <div className="bg-slate-900 w-full h-full absolute inset-0 pointer-events-none" />);
};
...
const StatBlock = ({ label, value, max, alert, inverted, onClick, subValue }) => (
  <button 
    onClick={onClick}
    className={`
      flex flex-col items-center justify-center w-12 h-12 md:w-16 md:h-16 bg-slate-800/60 rounded-xl border backdrop-blur-sm shadow-sm transition-all hover:scale-105 active:scale-95
      ${alert ? 'border-red-500/50 bg-red-900/20' : inverted ? 'border-amber-500/50 bg-amber-900/20' : 'border-slate-700/50 hover:border-slate-500'}
    `}
  >
      ...
      <span className={`text-xs md:text-lg font-bold font-mono ${alert ? 'text-red-300' : inverted ? 'text-amber-200' : 'text-slate-100'}`}>
        {Math.floor(value)}{max !== undefined && <span className="text-[9px] md:text-xs text-slate-500">/{max}</span>}
      </span>
      {subValue !== undefined && <span className="text-[9px] text-indigo-400 font-mono">+{subValue}</span>}
  </button>
);

const ActionButton = ({ icon, label, days, cost, costType = 'gp', onClick, disabled, description, effects }) => {
  const IconComponent = ICONS[icon] || HelpCircle;
  return (
    <button 
      onClick={onClick} disabled={disabled}
      className={`
        flex items-center gap-3 p-3 w-full rounded-xl border text-left transition-all relative overflow-hidden group
        ${disabled 
      ...
      `}
    >
      <div className={`p-2 rounded-lg ${disabled ? 'bg-slate-700' : 'bg-slate-900 group-hover:text-indigo-400'}`}>
        <IconComponent size={20} />
      </div>
      <div className="flex flex-col flex-1 min-w-0">
        <div className="flex justify-between items-center mb-0.5">
          <span className="font-bold text-sm truncate">{label}</span>
          {days > 0 && <span className="text-[10px] text-slate-400 flex items-center gap-1 font-mono"><Clock size={12}/> {days}d</span>}
        </div>
        <span className="text-xs text-slate-500 truncate leading-tight">{description}</span>
        {effects && renderEffectsList(effects)}
      </div>
      {cost > 0 && (
        <div className={`text-[10px] font-mono px-2 py-1 rounded ml-2 shadow-inner ${disabled ? 'bg-slate-700' : 'bg-black/40'} ${costType === 'gp' ? 'text-amber-400' : 'text-cyan-400'}`}>
          -{cost}{costType}
        </div>
      )}
    </button>
...
const CharacterSVG = ({ equipped, appearance, isAlive }) => {
  ...
  return (
    <svg viewBox="0 0 300 450" className={`w-full h-full transition-all duration-1000 ${isAlive ? 'drop-shadow-2xl' : 'grayscale opacity-50 blur-[1px]'}`}>
      <defs>
               ...
               <g>
                 ...
                 {equipped.body === 'plate' && (
                    <g stroke="#27272a" strokeWidth="1" fill="none">
                        <path d="M126 330 L 147 330" /><path d="M127 360 L 146 360" />
                        <path d="M153 330 L 174 330" /><path d="M154 360 L 173 360" />
                    </g>
                 )}
               </g>
             )}
      ...
      <g id="head">
        ...
        {!wearingFullHelm && (
           <g id="face-features">
             ...
             <circle cx="143" cy="60" r="2" fill={isAlive ? eyes : '#000'} stroke="black" strokeWidth="0.5" />
             <circle cx="157" cy="60" r="2" fill={isAlive ? eyes : '#000'} stroke="black" strokeWidth="0.5" />
      ...
      <g id="off-hand" transform="translate(95, 200) rotate(10)">
         ...
         {equipped.offHand === 'orb' && (
           <g transform="translate(0, -10)">
            ...
            <circle cx="0" cy="0" r="16" fill="none" stroke="#c7d2fe" strokeWidth="1" className="animate-[spin_4s_linear_infinite]" strokeDasharray="4 4"/>
           </g>
         )}
      </g>
    </svg>
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl relative overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="p-3 bg-indigo-900/20 rounded-lg border border-indigo-500/20 text-center">
                <span className="text-[10px] text-indigo-300 font-bold uppercase block mb-1">Current Status</span>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-950/30 flex justify-end">
            <button onClick={onClose} className="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg transition-colors shadow-lg shadow-indigo-500/20">
                Start Day
            </button>
        </div>
      </div>
                            ...
                            {Object.keys(attributes).map(attr => (
                                <div key={attr} className="flex items-center justify-between p-3 bg-slate-800/50 rounded border border-slate-700">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] text-slate-500">{getStatInfo(attr).desc}</span>
                                    </div>
...
const useGameLogic = () => {
  ...
  const currentStats = useMemo(() => {
    ...
    Object.keys(equipped).forEach(slot => {
      ...
      if (item && item.stats) Object.entries(item.stats).forEach(([stat, val]) => { total[stat] = (total[stat] || 0) + val; });
    });
    if (quirk && quirk.effects && quirk.effects.stats) Object.entries(quirk.effects.stats).forEach(([stat, val]) => { total[stat] = (total[stat] || 0) + val; });
    return total;
  ...
  const generateDailyQuests = (currentTier) => {
    const getPool = (db, tier) => { let pool = []; for (let i = 1; i <= tier; i++) if (db[`tier${i}`]) pool = [...pool, ...db[`tier${i}`]]; return pool; };
    const selectRandom = (pool, count) => (!pool || pool.length === 0) ? [] : [...pool].sort(() => 0.5 - Math.random()).slice(0, count);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setLocation(parsed.location || 'village_road'); setHousing(parsed.housing || 'homeless'); setRentActive(parsed.rentActive || false);
        setDays(parsed.days || 1); setInventory(parsed.inventory || ['none', 'tunic', 'fist']); setMaxTier(parsed.maxTier || 1);
        setDailyLogs(parsed.dailyLogs || []); setQuirk(parsed.quirk || null); 
        ...
        setGameStarted(true);
      } catch (e) { refreshShop(); setDailyQuests(generateDailyQuests(1)); }
  ...
  useEffect(() => {
      ...
      localStorage.setItem(SAVE_KEY, JSON.stringify({ attributes, stats, resources, equipped, appearance, location, inventory, shopStock, days, housing, rentActive, maxTier, dailyQuests, dailyLogs, quirk, lastSave: Date.now() }));
  ...
  const passTime = (daysPassed) => {
      let rentMsg = "No rent paid (Homeless)."; let changes = [];
      if (rentActive) {
          ...
          if (locId && LOCATIONS[locId]) {
             ...
             if (resources.gold >= totalRent) {
                  setResources(prev => ({ ...prev, gold: prev.gold - totalRent })); rentMsg = `Paid rent: -${totalRent}g at ${LOCATIONS[locId].name}.`; changes.push(`-${totalRent} Gold`);
                  if (LOCATIONS[locId].modifiers?.rest) {
                      ...
                      if(mod.health) changes.push(`${mod.health > 0 ? '+' : ''}${mod.health} Health`); if(mod.stress) changes.push(`${mod.stress > 0 ? '+' : ''}${mod.stress} Stress`); if(mod.mood) changes.push(`${mod.mood > 0 ? '+' : ''}${mod.mood} Mood`);
                  }
             } else {
                  setHousing('homeless'); setRentActive(false); setStats(prev => ({ ...prev, mood: Math.max(0, prev.mood - 20) })); rentMsg = "Evicted! Couldn't pay rent. Slept in the dirt."; changes.push("Evicted", "-20 Mood"); addMessage("Evicted!", 'error');
             }
          }
      } else {
      ...
      const incident = runAutonomyEvent(checkAutonomy(stats.mood, stats.stress));
      let incidentMsg = "Nothing purely chaotic happened.";
      if (incident) {
          incidentMsg = incident.text; const fx = incident.effects;
          if (fx) {
              ...
              if (fx.equipmentLoss) {
                  const slots = ['head', 'body', 'mainHand', 'offHand']; const randomSlot = slots[Math.floor(Math.random() * slots.length)]; const itemId = equipped[randomSlot];
                  if (itemId !== 'none' && itemId !== 'fist' && itemId !== 'tunic') { setEquipped(prev => ({ ...prev, [randomSlot]: 'none' })); incidentMsg += ` (Lost ${itemId})`; changes.push(`-${itemId}`); }
              }
          }
      ...
      setDays(prev => prev + daysPassed); refreshShop(); setDailyQuests(generateDailyQuests(maxTier));
  };
  ...
  const performAction = (action) => {
    ...
    if (action.id === 'rent_start') {
        if (resources.gold >= 5) { setHousing('inn'); setRentActive(true); setResources(prev => ({ ...prev, gold: prev.gold - 5 })); addMessage("Rented room at Rusty Spoon.", 'success'); addToLog({ type: 'action', day: days, title: 'Housing', text: 'Rented a room at the Rusty Spoon.', status: 'Success', changes: 'Changes: -5 Gold, +Warm Bed' }); } 
    ...
    if (action.id === 'rent_stop') { setHousing('homeless'); setRentActive(false); addMessage("Checked out of Inn.", 'info'); addToLog({ type: 'action', day: days, title: 'Housing', text: 'Checked out of the inn.', status: 'Info', changes: 'Changes: -Warm Bed' }); return; }
    ...
    if (action.id === 'eat' || action.id === 'drink') {
        ...
        if (ownedSupplies.length > 0) {
            const itemToConsume = ownedSupplies[0]; const idx = inventory.indexOf(itemToConsume.id);
            if (idx > -1) {
    ...
    if (action.type === 'labor') failChance = 0.40 - ((currentStats.str + currentStats.con) * 0.01) + (stats.stress * 0.002);
    else if (action.type === 'adventure') failChance = 0.60 - ((currentStats.str + currentStats.dex + currentStats.ac) * 0.01) + (stats.stress * 0.002);
    else if (action.type === 'social') failChance = 0.40 - (currentStats.cha * 0.02) + (stats.stress * 0.002);
    
    if (['labor', 'adventure', 'social'].includes(action.type) && Math.random() < Math.max(0.05, Math.min(0.95, failChance))) isSuccess = false;

    if (isSuccess) {
        let moodGain = action.effects.mood || 0;
        if (quirk && quirk.id === 'drama_queen' && moodGain > 0) moodGain *= (quirk.effects.moodMultiplier || 1);
        const [healthGain, hungerGain, thirstGain, stressGain] = [action.effects.health || 0, action.effects.hunger || 0, action.effects.thirst || 0, action.effects.stress || 0];
        ...
        if (action.effects.xp) {
          ...
          setResources(prev => {
            const currentXp = prev.xp + action.effects.xp; let currentLevel = prev.level;
            if (currentXp >= prev.level * 100) { currentLevel++; addMessage(`Level Up! Level ${currentLevel}`, "success"); lootText += " LEVEL UP!"; changes.push("LEVEL UP"); }
            return { ...prev, xp: currentXp, gold: prev.gold + goldGain, level: currentLevel };
          });
        }
        if (action.type === 'adventure' && Math.random() < 0.15) { 
           ...
           if (findableItems.length > 0) {
             ...
             if (!inventory.includes(foundItem.id)) { setInventory(prev => [...prev, foundItem.id]); addMessage(`Loot: Found ${foundItem.name}!`, 'success'); lootText += ` Found: ${foundItem.name}`; changes.push(`+${foundItem.name}`); }
           }
        }
        addToLog({ type: 'action', day: days, title: action.label, text: (action.message || "Completed action.") + lootText, status: 'Success', changes: changes.length > 0 ? `Changes: ${changes.join(", ")}` : "" });
    } else {
        let failMsg = action.type === 'labor' ? "Screwed up the job. No pay." : action.type === 'adventure' ? "Defeated! Retreated with wounds." : "Made a total fool of yourself.";
        let stressGain = action.type === 'labor' ? 10 : 0;
        if (action.type === 'adventure') { setStats(prev => ({ ...prev, health: Math.max(0, prev.health - 20), stress: Math.min(100, prev.stress + 20), hunger: Math.min(100, prev.hunger + 20), thirst: Math.min(100, prev.thirst + 20) })); changes.push("-20 Health", "+20 Stress", "+20 Hunger", "+20 Thirst"); } 
        else if (action.type === 'social') { setStats(prev => ({ ...prev, mood: Math.max(0, prev.mood - 20) })); changes.push("-20 Mood"); }
  ...
  const revive = () => {
    setStats({ health: maxStats.health, mood: maxStats.mood, hunger: 0, thirst: 0, stress: 0 }); setIsDead(false); setResources(prev => ({ ...prev, xp: Math.max(0, prev.xp - 50) })); setHousing('homeless'); setRentActive(false);
    addMessage("Revived... destitute.", "info"); addToLog({ type: 'action', day: days, title: 'Revived', text: 'Returned from the dead. Ouch.', status: 'Revived', changes: 'Changes: -50 XP, -Housing, +Life' });
  };

  const buyItem = (item) => {
    let cost = item.cost; if (quirk && quirk.id === 'lightweight' && item.type === 'drink') cost = Math.floor(cost * (quirk.effects.drinkCostMultiplier || 1));
    if (resources.gold >= cost) { setResources(prev => ({ ...prev, gold: prev.gold - cost })); setInventory(prev => [...prev, item.id]); addMessage(`Purchased ${item.name}`, 'success'); addToLog({ type: 'action', day: days, title: 'Shop', text: `Bought ${item.name}.`, status: 'Success', changes: `Changes: -${cost} Gold, +${item.name}` }); } 
    else addMessage("Not enough gold!", 'error');
  };

  const sellItem = (item) => {
    const sellValue = Math.floor(item.cost / 2); setResources(prev => ({ ...prev, gold: prev.gold + sellValue })); const idx = inventory.indexOf(item.id);
  ...
  const startGame = () => {
      setStats(prev => ({ ...prev, health: calculateMaxStats(1, attributes.con).health }));
      const randomQuirk = QUIRKS[Math.floor(Math.random() * QUIRKS.length)]; setQuirk(randomQuirk); setGameStarted(true); setDailyQuests(generateDailyQuests(1)); 
      setTimeout(() => alert(`You were born with a trait: ${randomQuirk.name}\n${randomQuirk.desc}`), 500);
  };

  const resetGame = () => { if (window.confirm("Reset game?")) { localStorage.removeItem(SAVE_KEY); window.location.reload(); } };

  return { gameStarted, creationStep, setCreationStep, attributes, updateAttribute, stats, resources, inventory, shopStock, equipped, equipItem, appearance, updateAppearance, days, location, housing, rentActive, dailyQuests, messages, isDead, maxStats, currentStats, dailyLogs, quirk, performAction, revive, buyItem, sellItem, consumeItem, startGame, resetGame, pointsAvailable: 10 - (Object.values(attributes).reduce((a, b) => a + b, 0) - 50) };
};


const STAT_DESCS = { str: 'Strength. Physical power.', dex: 'Dexterity. Agility and reflexes.', con: 'Constitution. Health and stamina.', int: 'Intelligence. Memory and reasoning.', cha: 'Charisma. Social influence.' };
...
export default function App() {
    ...
    useEffect(() => {
        if (game.days > lastDay) {
            const latestLog = game.dailyLogs[0];
            if (latestLog && latestLog.type === 'morning') setShowMorningReport(true);
            setLastDay(game.days);
        }
    }, [game.days, game.dailyLogs, lastDay]);

    if (!game.gameStarted) return <CreationScreen {...game} getStatInfo={(attr) => ({desc: STAT_DESCS[attr]})} />;

    const Background = getBackground(game.location);
    const latestMorningLog = game.dailyLogs.find(l => l.type === 'morning' && l.day === game.days - 1) || game.dailyLogs[0];
    const allItems = [...ITEM_DB.head, ...ITEM_DB.body, ...ITEM_DB.mainHand, ...ITEM_DB.offHand, ...ITEM_DB.supplies];

    return (
        <div className="relative w-full h-screen overflow-hidden text-slate-100 font-sans selection:bg-indigo-500/30">
            <Background />

            {/* Top HUD */}
            <header className="absolute top-0 w-full p-4 flex justify-between items-start z-40 pointer-events-none">
                <div className="flex gap-4 md:gap-8 bg-slate-900/80 backdrop-blur-md px-6 py-3 rounded-2xl border border-slate-700/50 shadow-xl pointer-events-auto">
                    <div className="flex flex-col">
                        <span className="text-[10px] text-amber-500/70 font-bold uppercase tracking-widest">Gold</span>
                        <span className="text-xl font-mono text-amber-400 drop-shadow-md">{game.resources.gold}</span>
                    </div>
                    <div className="w-px bg-slate-700/50" />
                    <div className="flex flex-col">
                        <span className="text-[10px] text-indigo-300/70 font-bold uppercase tracking-widest">Day</span>
                        <span className="text-xl font-mono text-slate-100">{game.days}</span>
                    </div>
                </div>
                <button onClick={game.resetGame} className="p-3 bg-red-950/80 border border-red-900/50 rounded-xl text-red-400 hover:bg-red-900 hover:text-white transition-all backdrop-blur-md pointer-events-auto">
                    <Trash2 size={18} />
                </button>
            </header>

            {/* Toasts */}
            <div className="absolute top-24 right-4 z-50 flex flex-col gap-2 items-end pointer-events-none">
                {game.messages.map(m => (
                    <div key={m.id} className={`px-4 py-2 rounded-lg shadow-xl text-xs font-bold tracking-wide border backdrop-blur-md animate-in slide-in-from-right fade-in duration-300 ${m.type === 'error' ? 'bg-red-950/90 border-red-500/50 text-red-200' : m.type === 'warning' ? 'bg-amber-950/90 border-amber-500/50 text-amber-200' : 'bg-slate-900/90 border-indigo-500/50 text-indigo-200'}`}>
                        {m.text}
                    </div>
                ))}
            </div>
            ...
            {/* Main Stage */}
            <main className="absolute inset-0 flex flex-col md:flex-row pt-24 pb-4 px-4 gap-4 z-10 pointer-events-none">
                
                {/* Left side: Character & Core Stats */}
                <div className="w-full md:w-1/3 flex flex-col items-center justify-start pointer-events-auto relative">
                    <div className="w-64 h-80 relative flex justify-center">
                        {game.quirk && (
                            <div className="absolute -top-4 z-20 flex items-center gap-1.5 px-3 py-1 bg-indigo-950/90 border border-indigo-500/50 rounded-full text-[10px] text-indigo-200 font-bold backdrop-blur-md shadow-xl whitespace-nowrap">
                                <Brain size={12} className="text-indigo-400" /> {game.quirk.name}
                            </div>
                        )}
                        <CharacterSVG equipped={game.equipped} appearance={game.appearance} isAlive={!game.isDead} />
                    </div>

                    <div className="flex flex-wrap justify-center gap-2 mt-2 bg-slate-900/80 p-3 rounded-2xl border border-slate-700/50 backdrop-blur-md shadow-xl w-full max-w-sm">
                        <StatBlock label="HP" value={game.stats.health} max={game.maxStats.health} alert={game.stats.health < 20} />
                        <StatBlock label="Mood" value={game.stats.mood} max={game.maxStats.mood} alert={game.stats.mood < 30} />
                        <StatBlock label="Hunger" value={game.stats.hunger} max={game.maxStats.hunger} alert={game.stats.hunger > 70} inverted />
                        <StatBlock label="Thirst" value={game.stats.thirst} max={game.maxStats.thirst} alert={game.stats.thirst > 70} inverted />
                        <StatBlock label="Stress" value={game.stats.stress} max={game.maxStats.stress} alert={game.stats.stress > 70} inverted />
                    </div>

                    {game.isDead && (
                        <div className="mt-4 p-4 bg-red-950/90 border border-red-900/50 rounded-2xl text-center backdrop-blur-md w-full max-w-sm shadow-[0_0_30px_rgba(220,38,38,0.3)]">
                            <Skull className="w-12 h-12 text-red-500 mx-auto mb-2 drop-shadow-lg" />
                            <h2 className="text-xl font-bold text-red-400 mb-2 uppercase tracking-widest">You Died</h2>
                            <button onClick={game.revive} className="w-full py-2 bg-red-900 hover:bg-red-800 text-white font-bold rounded-lg transition-colors border border-red-700">
                                Revive (-50 XP)
                            </button>
                        </div>
                    )}
                ...
                {/* Right side: Panels */}
                <div className="w-full md:w-2/3 bg-slate-900/90 backdrop-blur-xl border border-slate-700 rounded-3xl shadow-2xl flex flex-col pointer-events-auto overflow-hidden h-[calc(100vh-8rem)]">
                    
                    {/* Tabs */}
                    <div className="flex border-b border-slate-700/50 bg-slate-950/50">
                        {['actions', 'quests', 'shop', 'inventory', 'log'].map(tab => (
                            <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-4 text-[10px] md:text-xs font-bold uppercase tracking-wider transition-colors ${activeTab === tab ? 'text-indigo-400 border-b-2 border-indigo-400 bg-indigo-950/20' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/30'}`}>
                                {tab}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                        ...
                        {activeTab === 'actions' && (
                            <div className="space-y-4 animate-in fade-in">
                                <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-4">
                                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest block mb-3">Housing Status</span>
                        ...
                        {activeTab === 'quests' && (
                            <div className="space-y-4 animate-in fade-in">
                                <div>
                                    <span className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest block pl-2 mb-2">Daily Labor</span>
                                    <div className="space-y-2">{game.dailyQuests.labor.map(action => <ActionButton key={action.id} {...action} onClick={() => game.performAction(action)} disabled={game.isDead} />)}</div>
                                </div>
                                <div>
                                    <span className="text-[10px] text-red-400 font-bold uppercase tracking-widest block pl-2 mb-2">Adventures</span>
                                    <div className="space-y-2">{game.dailyQuests.adventure.map(action => <ActionButton key={action.id} {...action} onClick={() => game.performAction(action)} disabled={game.isDead} />)}</div>
                                </div>
                                <div>
                                    <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest block pl-2 mb-2">Socializing</span>
                                    <div className="space-y-2">{game.dailyQuests.social.map(action => <ActionButton key={action.id} {...action} onClick={() => game.performAction(action)} disabled={game.isDead} />)}</div>
                                </div>
                            </div>
                        )}
                        ...
                        {activeTab === 'shop' && (
                            <div className="space-y-4 animate-in fade-in">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    {game.shopStock.map(itemId => {
                                        const item = allItems.find(i => i.id === itemId);
                                        if (!item) return null;
                                        return (
                                            <div key={item.id} className="bg-slate-800/80 border border-slate-700 rounded-xl p-3 flex flex-col justify-between hover:border-slate-500 transition-colors">
                                                <div className="mb-3">
                                                    <div className="flex justify-between items-start mb-1">
                                                        <span className="font-bold text-sm text-slate-200">{item.name}</span>
                                                        <span className="text-[9px] text-slate-400 uppercase tracking-widest bg-slate-900 px-1.5 py-0.5 rounded">{item.category}</span>
                                                    </div>
                                                    <p className="text-[10px] text-slate-500 mb-2">{item.description}</p>
                                                    {renderEffectsList(item.stats || item.effects)}
                                                </div>
                                                <button onClick={() => game.buyItem(item)} disabled={game.isDead || game.resources.gold < item.cost} className="w-full py-2 bg-amber-900/30 hover:bg-amber-800 text-amber-400 border border-amber-900/50 rounded-lg text-xs font-bold transition-colors disabled:opacity-50 flex justify-between px-4">
                                                    <span>Buy Item</span>
                                                    <span className="font-mono">{item.cost}g</span>
                                                </button>
                                            </div>
                                        );
                                    })}
                        ...
                        {activeTab === 'inventory' && (
                            <div className="space-y-6 animate-in fade-in">
                                <div>
                                    <h3 className="text-[10px] text-indigo-400 font-bold uppercase tracking-widest block pl-2 mb-3">Equipped Gear</h3>
                                    <div className="grid grid-cols-2 gap-3">
                                        {['head', 'body', 'mainHand', 'offHand'].map(slot => {
                                            const item = ITEM_DB[slot].find(i => i.id === game.equipped[slot]);
                                            return (
                                                <div key={slot} className="bg-slate-950/50 border border-slate-800 rounded-xl p-3">
                                                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-widest block mb-1">{slot.replace('Hand', ' Hand')}</span>
                                                    <span className="text-sm font-bold text-indigo-200">{item?.name || 'Empty'}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                                ...
                                <div className="h-px bg-slate-800 w-full" />
                                <div>
                                    <h3 className="text-[10px] text-slate-400 font-bold uppercase tracking-widest block pl-2 mb-3">Backpack</h3>
                                    <div className="space-y-2">
                                        {game.inventory.filter(id => !Object.values(game.equipped).includes(id)).map((itemId, idx) => {
                                            const item = allItems.find(i => i.id === itemId);
                                            if (!item) return null;
                                            return (
                                                <div key={`${item.id}-${idx}`} className="bg-slate-800/50 border border-slate-700 rounded-xl p-3 flex justify-between items-center group">
                                                    <div className="flex flex-col">
                                                        <span className="font-bold text-sm text-slate-200">{item.name}</span>
                                                    ...
                                                    <div className="flex gap-2">
                                                        {item.type === 'food' || item.type === 'drink' || item.type === 'potion' ? (
                                                            <button onClick={() => game.consumeItem(item)} disabled={game.isDead} className="px-3 py-1.5 bg-emerald-900/30 hover:bg-emerald-800 text-emerald-400 border border-emerald-900/50 rounded-lg text-[10px] font-bold transition-colors">Consume</button>
                                                        ) : (
                                                            <button onClick={() => game.equipItem(item)} disabled={game.isDead} className="px-3 py-1.5 bg-indigo-900/30 hover:bg-indigo-800 text-indigo-400 border border-indigo-900/50 rounded-lg text-[10px] font-bold transition-colors">Equip</button>
                                                        )}
                                                        <button onClick={() => game.sellItem(item)} disabled={game.isDead} className="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-lg text-[10px] font-bold transition-colors">Sell (+{Math.floor(item.cost/2)}g)</button>
                                                    </div>
                                                </div>
                                            );
                                        })}
                                        {game.inventory.filter(id => !Object.values(game.equipped).includes(id)).length === 0 && (
                                            <div className="text-center p-6 text-slate-500 text-xs uppercase tracking-widest font-bold border border-dashed border-slate-800 rounded-xl">Backpack is empty.</div>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        {activeTab === 'log' && (
                            <div className="space-y-3 animate-in fade-in">
                                {game.dailyLogs.length === 0 ? (
                                    ...
                                    <div key={log.id} className="bg-slate-900/60 rounded-xl border border-slate-700/50 overflow-hidden backdrop-blur-sm">
                                    {log.type === 'morning' ? (
                                        <>
                                        ...
                                        <div className="p-4">
                                            <div className="flex justify-between items-start mb-2">
                                                <span className="text-xs font-bold text-indigo-300 flex items-center gap-2">
                                                    <span className={log.status === 'Success' ? 'text-emerald-400' : log.status === 'Info' ? 'text-blue-400' : 'text-red-400'}>●</span> {log.title}
                                                </span>
                                                <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Day {log.day}</span>
                                            </div>
                                            <p className="text-sm text-slate-400 leading-relaxed mb-1">{log.text}</p>
                                            {log.changes && <p className="text-[10px] text-slate-500 font-mono mt-2">{log.changes}</p>}
                                        </div>
                                    )}
                                    </div>
                                ))}
            ...
            {showMorningReport && latestMorningLog && (
                <MorningReport log={latestMorningLog} onClose={() => setShowMorningReport(false)} />
            )}
        </div>
    );
}
